...
export class NetlistBuilder {
    build(input) {
        ...
        const railGroups = new Map(); // railRow → first pin key on that rail
        for (const key of uf.keys()) {
            const [row] = key.split(':');
            if (!row || !RAIL_ROWS.includes(row))
                continue;
...
function gaussElim(A, b) {
    ...
    for (let col = 0; col < n; col++) {
        ...
        if (Math.abs(piv) < 1e-15)
            return null; // singular / floating node
        for (let row = col + 1; row < n; row++) {
...
export class MNASolver {
    solve(netlist) {
        ...
        const voltSrcs = elements.filter((e) => e.type === 'V');
...
//# sourceMappingURL=MNASolver.js.map