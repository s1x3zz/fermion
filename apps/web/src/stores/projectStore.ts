...
export class ProjectLimitError extends Error {
  constructor(public tier: UserTier, public limit: number) {
...
function emptyCircuit(): BreadboardCircuit {
  return { components: [], wires: [] }
}

function newProject(name: string, description?: string): FermionProject {
  const now = Date.now()
  return {
...
async function pushToCloud(project: FermionProject): Promise<void> {
  const { user } = getAuthSnapshot()
  if (!user) return
...
async function fetchFromCloud(): Promise<FermionProject[]> {
  const { user } = getAuthSnapshot()
  if (!user) return []
  ...
  return (data ?? []).map((row) => ({
    ...
    circuit: row.circuit as BreadboardCircuit,
...
interface ProjectState {
  projects: FermionProject[]
  activeProject: FermionProject | null
  ...
  loadDraft: () => Promise<{ circuit: BreadboardCircuit; savedAt: number } | null>
  clearDraft: () => Promise<void>
}

const _store = createStore<ProjectState>()((set, get) => ({
  projects: [],
  activeProject: null,
  ...
  createProject: async (name, description) => {
    ...
    if (projects.length >= limit) throw new ProjectLimitError(tier, limit)
    ...
    if (cloudSyncEnabled && tier !== 'guest') {
      set({ syncing: true })
      try { await pushToCloud(project) } finally { set({ syncing: false }) }
    }
  ...
  saveProject: async (project) => {
    ...
    if (cloudSyncEnabled && tier !== 'guest') {
      set({ syncing: true })
      try { await pushToCloud(updated) } finally { set({ syncing: false }) }
    }
  ...
  deleteProject: async (id) => {
    ...
    if (cloudSyncEnabled && tier !== 'guest') {
      set({ syncing: true })
      try { await deleteFromCloud(id) } finally { set({ syncing: false }) }
    }
  ...
  toggleCloudSync: async () => {
    ...
    if (next) await get().loadProjects()
  },

  importProject: async (file) => {
    const text = await file.text()
    let raw: unknown
    try { raw = JSON.parse(text) } catch { throw new Error('Invalid JSON file') }

    const result = FermionProjectSchema.safeParse(raw)
    if (!result.success) throw new Error(`Invalid project file: ${result.error.issues[0]?.message}`)
    ...
    const d = result.data
    const project: FermionProject = {
      id: crypto.randomUUID(),
      name: d.name,
      createdAt: d.createdAt,
      updatedAt: Date.now(),
      circuit: d.circuit as BreadboardCircuit,
      metadata: d.metadata,
...
const [projects, setProjects] = createSignal<FermionProject[]>([])
const [activeProject, setActiveProjectSignal] = createSignal<FermionProject | null>(null)
