              ...
              {(item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (!locked) props.onPlace(item.type)
                      else if (props.onUpgradePrompt) props.onUpgradePrompt()
                    }}
                    title={locked ? 'Upgrade to Pro to use this component' : `Place ${item.label}`}
                  >
                    <div
                      class="comp-icon"
                      style={{ background: locked ? '#1a1a2e' : item.color }}
                    />
        ...
        <For each={LIBRARY}>
          {(section) => (
            <Section
              ...
              filter={search()}
              onPlace={handlePlace}
              onUpgradePrompt={props.onUpgradePrompt}
            />
          )}
        </For>
      </div>
