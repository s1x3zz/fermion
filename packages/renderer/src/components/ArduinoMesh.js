import { BoxGeometry, CylinderGeometry, SphereGeometry, MeshStandardMaterial, MeshBasicMaterial, Mesh, Vector3, Color, } from 'three';
...
export class ArduinoMesh extends ComponentMesh {
    ...
    _build() {
        ...
        const createHeaderRow = (count, startX, startZ, spacing, pinOffset, isAnalog = false) => {
    ...
    setLedState(led, state) {
        ...
        if (mesh) {
            mesh.material.color.setHex(state ? (led === 'L' ? 0xffff00 : 0xffa500) : 0x000000);
        }
    }
    getAllPins() {
        const pins = [];
...
//# sourceMappingURL=ArduinoMesh.js.map