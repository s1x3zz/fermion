...
const LED_HEX: Partial<Record<ComponentType, number>> = {
  led_red:    0xff2020,
  led_green:  0x20ff40,
  led_blue:   0x2040ff,
  led_yellow: 0xffcc00,
...
export class LedMesh extends ComponentMesh {
  private _baseMat!: MeshStandardMaterial
  private _domeMat!: MeshPhysicalMaterial
  ...
  private _build(): void {
    ...
    const anodeLead = new Mesh(
      new CylinderGeometry(LEAD_R, LEAD_R, LEAD_LEN + anodeExtraLen, 6),
      leadMat,
    )
  ...
  setGlow(glowing: boolean, _current = 0): void {
