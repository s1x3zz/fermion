...
const TESTIMONIALS = [
    {
        quote: "The most elegant circuit simulator I've ever used. MNA solving in real-time is basically witchcraft.",
        author: 'Alex Chen',
        role: 'Senior Hardware Engineer · Google',
        initials: 'AC',
    },
    {
        quote: "Finally replaced LTspice for rapid prototyping. The 3D PCB view alone justifies the switch.",
        author: 'María García',
...
const truncate = (s, max = 22) => (s.length > max ? s.slice(0, max - 1) + '…' : s);
// Highlighted Arduino code (safe static string — no user input)
const CODE_HTML = '<span class="ck">void</span> <span class="cf">setup</span>() {\n' +
...
function BreadboardViz() {
    ...
    return (<div class="bb-wrap">
      <svg viewBox="0 0 260 130" class="bb-svg" aria-hidden="true">
        ...
        {[35, 55, 75, 95].map((cy) => [28, 48, 68, 88, 108, 128, 148, 168, 188, 208, 228].map((cx) => (<circle cx={cx} cy={cy} r="2.4" fill="#101e10" stroke="#1e3a1e" stroke-width="0.4"/>)))}
        {/* Power rails */}
        ...
        {/* Traces */}
        <polyline points="28,35 48,35" fill="none" stroke="#3b8bff" stroke-width="1" opacity="0.5"/>
        <polyline points="96,35 148,35 148,55" fill="none" stroke="#3b8bff" stroke-width="1" opacity="0.5"/>
        <polyline points="28,55 108,55 108,75 148,75" fill="none" stroke="#a78bfa" stroke-width="1" opacity="0.45"/>
        {/* LED1 — blue, pulsing */}
        ...
        <circle r="2.8" fill="#3b8bff" opacity="0.85">
          <animateMotion dur="2.8s" repeatCount="indefinite" calcMode="linear" path="M 28 35 L 48 35 L 60 35 L 96 35 L 148 35 L 148 55"/>
        </circle>
        <circle r="2.2" fill="#a78bfa" opacity="0.8">
          <animateMotion dur="3.2s" repeatCount="indefinite" calcMode="linear" begin="1.2s" path="M 28 55 L 108 55 L 108 75 L 148 75"/>
        </circle>
      </svg>
...
function ThemeButton() {
    ...
    return (<button class="nav-theme-btn" onClick={openThemePalette} title="Change theme (Ctrl+T)" aria-label="Open theme palette">
...
function Hero(props) {
    ...
    return (<section class="hero">
      ...
      <div class="hero-inner l-inner">
        ...
        <div class="hero-btns">
          <button class="btn btn-primary btn-lg" onClick={() => (auth.user() ? navigate('/dashboard') : navigate('/sim?guest=true'))}>
...
function SchematicView() {
    return (<div class="cmp-schem-wrap">
      <svg viewBox="0 0 360 180" class="cmp-schem-svg" aria-hidden="true">
        ...
        <polyline points="50,73 50,40 100,40 175,40 205,40 248,40 310,40 310,140 50,140 50,107" fill="none" stroke="#0e1430" stroke-width="2"/>
        ...
        <polyline class="schem-current" points="50,73 50,40 100,40 175,40 205,40 248,40 310,40 310,140 50,140 50,107" fill="none" stroke="#3b8bff" stroke-width="2" stroke-dasharray="10 16" opacity="0.9"/>
        ...
        <rect x="100" y="32" width="75" height="16" rx="2" fill="#060614" stroke="#3b8bff" stroke-width="1.3" opacity="0.9"/>
        ...
        <polygon points="205,24 205,56 248,40" fill="#060614" stroke="#3b8bff" stroke-width="1.3" stroke-linejoin="round"/>
...
function Demo() {
    ...
    const getRelPos = (clientX) => {
        const rect = containerRef.getBoundingClientRect();
        return clamp(((clientX - rect.left) / rect.width) * 100);
    };
    const onMouseMove = (e) => { if (dragging)
        setPos(getRelPos(e.clientX)); };
    const onMouseUp = () => { dragging = false; };
    const onTouchMove = (e) => {
        if (dragging && e.touches[0])
            setPos(getRelPos(e.touches[0].clientX));
    };
    ...
    return (<section class="section">
      <div class="l-inner">
        ...
        <div ref={containerRef} class="cmp-wrap reveal">
          ...
          <div class="cmp-panel cmp-left" style={{ 'clip-path': `inset(0 ${(100 - pos()).toFixed(2)}% 0 0)` }}>
            <SchematicView />
          </div>
          {/* Right panel — 3D simulation */}
          <div class="cmp-panel cmp-right" style={{ 'clip-path': `inset(0 0 0 ${pos().toFixed(2)}%)` }}>
            <View3D />
          </div>
          {/* Divider */}
          <div class="cmp-divider" style={{ left: `${pos().toFixed(2)}%` }}>
            <div class="cmp-line"/>
            <div class="cmp-handle" onMouseDown={() => { dragging = true; }} onTouchStart={() => { dragging = true; }}>
              ↔
            </div>
          </div>
        </div>
...
function Pricing(props) {
    ...
    const handlePlanClick = (plan) => {
        if (plan.name === 'Free')
            ...
            window.location.href = 'mailto:fermiondev.io@gmail.com';
    };
    return (<section class="section section-pricing" id="pricing">
      <PricingWaveCanvas />
      <div class="l-inner">
        <div class="section-head reveal">
        ...
        <div class="pricing-grid">
          <For each={PLANS}>
            {(plan, i) => (<div class={`pricing-card reveal${plan.featured ? ' pricing-featured' : ''}`} style={`--dl:${i() * 0.15}s`}>
                ...
                <button class={`btn plan-btn${plan.featured ? ' btn-primary' : ' btn-outline'}`} onClick={() => handlePlanClick(plan)}>
                  {plan.cta}
                </button>
              </div>)}
          </For>
      ...
      <div class="l-inner">
        ...
        <div class="testi-grid">
          <For each={TESTIMONIALS}>
            {(t, i) => (<div class="testi-card reveal" style={`--dl:${i() * 0.2}s`}>
...
function CTA(props) {
    ...
    return (<section class="cta-section">
      ...
      <div class="l-inner cta-inner reveal">
        ...
        <button class="btn btn-primary btn-cta" onClick={() => (auth.user() ? navigate('/dashboard') : props.onOpenModal())}>
      ...
      <div class="footer-inner l-inner">
        <div class="footer-brand">
          ...
          <div class="footer-social">
            <a href="https://github.com/s1x3zz/fermion" target="_blank" rel="noopener noreferrer" class="footer-gh" aria-label="GitHub">
...
export function Landing() {
    ...
    const [modalTab, setModalTab] = createSignal('signup');
    const [searchParams] = useSearchParams();
    useRevealObserver();
    onMount(() => {
        if (searchParams.login === '1') {
            setModalTab('login');
            setShowModal(true);
        }
    });
    ...
    return (<>
      ...
      <Footer />
      <Show when={showModal()}>
        <AuthModal onClose={close} initialTab={modalTab()} {...(searchParams.error !== undefined ? { initialError: searchParams.error } : {})}/>
      </Show>
      <ToastContainer />
    </>);
}
