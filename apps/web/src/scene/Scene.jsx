...
import { useCameraController, WireRenderer, SignalType, Breadboard, ComponentRegistry, } from '@fermion/renderer';
export function Scene(props) {
    ...
    onMount(() => {
        ...
        const toNdc = (clientX, clientY) => {
            ...
            ndcPoint.set(((clientX - rect.left) / rect.width) * 2 - 1, -((clientY - rect.top) / rect.height) * 2 + 1);
        };
        ...
        const showGhost = (worldX, worldZ) => {
            ...
            if (meshMap.has(snap.row)) {
                pinPos = meshMap.get(snap.row).getPinHeaderPosition(snap.col) ?? board.getPinPosition('a', 1);
            }
            else {
        ...
        scene.add(startRing);
        const startWiringMode = (snap) => {
            let startPos;
            if (meshMap.has(snap.row)) {
                startPos = meshMap.get(snap.row).getPinHeaderPosition(snap.col) ?? board.getPinPosition('a', 1);
            }
            else {
        ...
        const onMouseMove = (e) => {
            ...
            if (wiringState.mode === 'WIRING') {
                ...
                if (snap) {
                    if (meshMap.has(snap.row)) {
                        endPos = meshMap.get(snap.row).getPinHeaderPosition(snap.col) ?? board.getPinPosition('a', 1);
                    }
                    else {
        ...
        const onDrop = (e) => {
            e.preventDefault();
            const type = e.dataTransfer?.getData('fermion/component');
            if (!type || !ghostSnap) {
                hideGhost();
                return;
            }
            const { row, col } = ghostSnap;
            if (props.isOccupied?.(row, col)) {
                hideGhost();
                return;
            }
        ...
        const onPointerUp = (e) => {
            ...
            if (compHits.length > 0) {
                let node = compHits[0].object;
                while (node) {
                    const id = node.userData['componentId'];
                    if (id) {
                        props.onComponentSelect?.(id);
                        return;
                    }
                    node = node.parent;
                }
            ...
            if (wireHits.length > 0) {
                const wireId = wireHits[0].object.userData['wireId'];
                if (wireId) {
                    props.onWireSelect?.(wireId);
                    return;
                }
            }
        ...
        const api = {
            ...
            setSelectedComponent: (id) => {
                if (selectedMeshId)
                    meshMap.get(selectedMeshId)?.setSelected(false);
                selectedMeshId = id;
                if (id)
                    meshMap.get(id)?.setSelected(true);
            },
            removeWire: (id) => {
                wires.removeWire(id);
            ...
            setSelectedWire: (id) => {
                if (selectedWireId)
                    wires.setSelected(selectedWireId, false);
                selectedWireId = id;
                if (id)
                    wires.setSelected(id, true);
            },
            cancelWiring: () => {
                cancelWiringMode();
            },
            ...
            resetGlow: () => {
                ...
                for (const wireMesh of wires.getMeshes()) {
                    const id = wireMesh.userData['wireId'];
                    if (id)
                        wires.updateCurrent(id, 0);
                }
            },
                ...
                for (const wire of Object.values(wireData)) {
                    try {
                        ...
                        if (meshMap.has(wire.pinA.row)) {
                            const mesh = meshMap.get(wire.pinA.row);
                        ...
                        if (meshMap.has(wire.pinB.row)) {
                            const mesh = meshMap.get(wire.pinB.row);
                        ...
                        wires.addWire(wire.id, {
                            pinA,
                            pinB,
                            signalType: wire.signalType ?? SignalType.GENERIC,
                        });
                    }
                    catch {
