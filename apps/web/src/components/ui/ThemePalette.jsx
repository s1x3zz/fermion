...
export function ThemePalette(props) {
    ...
    function handleKeyDown(e) {
        ...
        if (e.key === 'ArrowDown') {
            ...
            if (theme && canUseTheme(theme.id, tier()))
    ...
    function handleBackdropClick(e) {
        if (e.target.classList.contains('tp-backdrop')) {
            setHovered(null);
            props.onClose();
        }
    }
      ...
      <Portal>
        <div class="tp-backdrop" onClick={handleBackdropClick}>
          <div class="tp-panel" role="dialog" aria-label="Theme selector">
            ...
            return (<button class="tp-item" classList={{
                    'tp-item--active': active(),
                    'tp-item--focused': focused() === i(),
                    'tp-item--locked': locked(),
                }} onClick={() => !locked() && handleSelect(theme.id)} onMouseEnter={() => { setHovered(theme.id); setFocused(i()); }} onMouseLeave={() => setHovered(null)} disabled={locked()} aria-pressed={active()}>
                      <div class="tp-preview-swatch" style={{
                    background: `linear-gradient(135deg, ${theme.previewColors[0]} 40%, ${theme.previewColors[1]} 100%)`,
                }}>
                        <span class="tp-swatch-dot" style={{ background: theme.previewColors[2] }}/>
                      </div>
            ...
            <Show when={previewTheme() === 'spatial' || currentTheme() === 'spatial'}>
              <div class="tp-accent-section">
                ...
                <div class="tp-accent-swatches">
                  <For each={Object.entries(ACCENT_COLORS)}>
                    {([key, val]) => (<button class="tp-accent-dot" classList={{ 'tp-accent-dot--active': accentColor() === key }} style={{ background: val.color }} title={val.label} onClick={() => setAccent(key)} aria-label={val.label} aria-pressed={accentColor() === key}/>)}
                  </For>
                </div>
              </div>
            </Show>
...
//# sourceMappingURL=ThemePalette.jsx.map