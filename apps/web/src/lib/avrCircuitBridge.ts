...
let currentPinStates: any = { digital: {}, analog: {}, pwm: {} };
let stateCallback: ((states: any, arduinos: string[]) => void) | null = null;

export function onAvrStateUpdate(cb: (states: any, arduinos: string[]) => void) {
  stateCallback = cb;
}
  ...
  bridgeInterval = setInterval(async () => {
    ...
    for (const arduino of arduinos) {
      for (let pin = 0; pin <= 19; pin++) { 
        const wires = Object.values(circuit.wires).filter(w => 
          (w.pinA.row === arduino.id && w.pinA.col === pin) ||
...
export function injectAvrComponents(
  components: Record<string, PlacedComponent>,
  wires: Record<string, PlacedWire>
): { components: Record<string, PlacedComponent>, wires: Record<string, PlacedWire> } {
  ...
  for (const arduino of arduinos) {
    for (let pin = 0; pin <= 19; pin++) {
      ...
      const isConnected = Object.values(wires).some(w => 
        (w.pinA.row === arduino.id && w.pinA.col === pin) ||
        (w.pinB.row === arduino.id && w.pinB.col === pin)
      );
