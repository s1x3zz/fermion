...
function getUserTierLabel(user: ReturnType<typeof useAuthStore>['user']): UserTier {
  const u = user()
  if (!u) return 'guest'
...
function NewProjectModal(props: { onClose: () => void; onCreate: (name: string, description?: string) => Promise<void> }) {
...
function LimitModal(props: { tier: UserTier; limit: number; onClose: () => void }) {
  function handleKeyDown(e: KeyboardEvent) {
    if (e.key === 'Escape') props.onClose()
  }
  onMount(() => window.addEventListener('keydown', handleKeyDown))
...
function DeleteModal(props: { project: FermionProject; onConfirm: () => void; onClose: () => void }) {
  function handleKeyDown(e: KeyboardEvent) {
    if (e.key === 'Escape') props.onClose()
  }
  onMount(() => window.addEventListener('keydown', handleKeyDown))
...
function ProjectCard(props: {
  project: FermionProject
  onOpen: () => void
  onDelete: () => void
  onExport: () => void
}) {
  return (
    <div class="project-card" onClick={props.onOpen} role="button" tabIndex={0}
...
export default function Dashboard() {
  ...
  const [limitInfo, setLimitInfo] = createSignal<{ tier: UserTier; limit: number } | null>(null)
  const [deleteTarget, setDeleteTarget] = createSignal<FermionProject | null>(null)
    ...
    <div class="dashboard">
          ...
          <Show when={isPaidUser()}>
            <label class="sync-toggle" title="Sync projects to cloud">
              <input
                ...
                onChange={store.toggleCloudSync}
              />
              Cloud sync
            </label>
          </Show>
      ...
      <main class="dashboard-main">
        ...
        <div class="project-grid">
          ...
          <div class="project-card-new" role="button" tabIndex={0}
            onClick={handleNewProjectClick}
            onKeyDown={(e) => e.key === 'Enter' && handleNewProjectClick()}>
          ...
          <For each={store.projects()}>
            {(project) => (
              <ProjectCard
                ...
                onExport={() => store.exportProject(project.id)}
              />
            )}
          </For>
        </div>
