import { Vector3, Group, BoxGeometry, CylinderGeometry, MeshStandardMaterial, Mesh, InstancedMesh, Object3D, } from 'three';
...
// ── Column X helpers ─────────────────────────────────────────────────────────
function _mainColX(col) {
    return ((col - 1) - (MAIN_COLS - 1) / 2) * PITCH;
}
function _railColX(col) {
...
export class Breadboard {
    ...
    disposables = [];
    constructor(scene, position = new Vector3()) {
        this.scene = scene;
    ...
    _buildHoles() {
        ...
        const railZs = [Z_VCC_TOP, Z_GND_TOP, Z_VCC_BOT, Z_GND_BOT];
    ...
    getPinPosition(row, col) {
        ...
        if (z === undefined)
            throw new Error(`Breadboard: unknown row "${row}"`);
        return new Vector3(this.group.position.x + _mainColX(col), this.group.position.y + BOARD_TOP_Y, this.group.position.z + z);
    }
    /**
     ...
     */
    snapToNearestPin(worldX, worldZ) {
        ...
        if (col < 1 || col > MAIN_COLS)
            return null;
        // Nearest row
        const rows = Object.entries(Z_ROW_MAP);
    ...
    getPowerRailPosition(rail, col) {
        ...
        return new Vector3(this.group.position.x + _railColX(col), this.group.position.y + BOARD_TOP_Y, this.group.position.z + zMap[rail]);
    }
    dispose() {
...
//# sourceMappingURL=Breadboard.js.map