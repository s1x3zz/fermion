...
function SimResultsPanel(props: { compId: string }) {
  ...
  const fmt = (v: number | null, unit: string, scale = 1) =>
    v === null ? '—' : `${(v * scale).toFixed(2)} ${unit}`
...
function ComponentInspector(props: { id: string; onDelete: () => void; onOpenSketch: (id: string) => void }) {
  const sim = useSimStore()
  const circuit = useCircuitStore()
  ...
  const [rotation, setRotation] = createSignal<0 | 90 | 180 | 270>(comp()?.rotation ?? 0)
  ...
  function handleLabelBlur() {
    const c = comp()
    if (c) circuit.updateComponent(c.id, { label: labelInput() || undefined })
  }

  function handleValueBlur() {
    const c = comp()
    const n = parseFloat(valueInput())
    if (c && !isNaN(n)) circuit.updateComponent(c.id, { value: n })
  }

  function handleRotationChange(v: string) {
    const r = Number(v) as 0 | 90 | 180 | 270
    setRotation(r)
    const c = comp()
    if (c) circuit.updateComponent(c.id, { rotation: r })
  }
        ...
        <div class="inspector-field">
          ...
          <div class="color-row">
            <For each={LED_COLORS}>
              {(c) => (
                <div
                  class={`color-swatch${ledColor() === c ? ' selected' : ''}`}
                  style={{ background: c }}
...
function WireInspector(props: { id: string; onDelete: () => void }) {
  const sim = useSimStore()
  const circuit = useCircuitStore()
  ...
  createEffect(() => {
    ...
    if (st) setSignalType(st)
  })

  const currentColor = () => SIGNAL_TYPES.find((s) => s.value === signalType())?.color ?? '#888'

  function handleSignalTypeChange(v: string) {
    setSignalType(v)
    circuit.updateWire(props.id, { signalType: v })
  }

  const pinLabel = (pin: { row: string; col: number } | undefined) =>
    pin ? `${pin.row.toUpperCase()}${pin.col}` : '—'
    ...
    <div>
      ...
      <div class="inspector-field">
        ...
        <select
          class="inspector-select"
          value={signalType()}
          onChange={(e) => handleSignalTypeChange(e.currentTarget.value)}
        >
...
export function Inspector(props: { onDeleteSelected: () => void; onOpenSketch: (id: string) => void }) {
  const sim = useSimStore()
  ...
  return (
    <div class="sim-inspector">
      ...
      <div class="inspector-body">
        <Show
          when={sim.selectedComponentId()}
          fallback={
            <Show when={sim.selectedWireId()} fallback={<EmptyState />}>
