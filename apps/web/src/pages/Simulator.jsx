...
export default function Simulator() {
    ...
    const [sceneApiSig, setSceneApiSig] = createSignal(null);
    ...
    const [sketchEditorTarget, setSketchEditorTarget] = createSignal(null);
    const [pendingDraftCircuit, setPendingDraftCircuit] = createSignal(null);
    const [pendingCircuitLoad, setPendingCircuitLoad] = createSignal(null);
    const userTier = () => {
        const u = auth.user();
        if (!u)
            return 'guest';
    ...
    async function handleSaveWithName(name) {
        const serialized = serializeCircuit(circuit.components(), circuit.wires());
        const thumb = sceneApiSig()?.captureThumb();
        try {
    ...
    function handleCursorMove(x, z) {
        sim.updateCursor(x, z);
    }
    function handleComponentPlace(type, row, col) {
        const id = circuit.addComponent(type, row, col);
        setIsDirty(true);
        return id;
    }
    function handleComponentSelect(id) {
        if (id)
            sim.selectComponent(id);
        else
            sim.deselect();
    }
    function handleWireCreate(pinA, pinB, signalType) {
        const id = circuit.addWire(pinA, pinB, signalType);
        setIsDirty(true);
        return id;
    }
    function handleWireSelect(id) {
        if (id)
            sim.selectWire(id);
        else
            sim.deselect();
    ...
    return (<div class="sim-layout">
      <Toolbar controller={controller} radius={radius} isDirty={isDirty} onSave={handleSave}/>
      ...
      <div class="sim-canvas-cell">
        ...
        <Scene onControllerReady={setController} onFrame={handleFrame} onCursorMove={handleCursorMove} isSimulating={sim.isSimulating} onSceneReady={setSceneApiSig} onComponentPlace={handleComponentPlace} onComponentSelect={handleComponentSelect} isOccupied={circuit.isOccupied} onWireCreate={handleWireCreate} onWireSelect={handleWireSelect}/>
      </div>
      ...
      <Show when={showSaveModal()}>
        <SaveModal onSave={handleSaveWithName} onCancel={() => setShowSaveModal(false)}/>
      </Show>
...
//# sourceMappingURL=Simulator.jsx.map